...
class Inception_Block(nn.Module):
    def __init__(self, in_channels, ch1x1, ch3x3reduce, ch3x3, ch5x5reduce, ch5x5, pool_proj):
        ...
        # 1x1 conv branch
        self.branch1 = BaseConvBlock(in_channels, ch1x1, kernel_size=1)

        # 1x1 conv -> 3x3 conv branch
        self.branch2 = nn.Sequential(
            BaseConvBlock(in_channels, ch3x3reduce, kernel_size=1),
            BaseConvBlock(ch3x3reduce, ch3x3, kernel_size=3, padding=1)
        )

        # 1x1 conv -> 5x5 conv branch
        self.branch3 = nn.Sequential(
            BaseConvBlock(in_channels, ch5x5reduce, kernel_size=1),
            BaseConvBlock(ch5x5reduce, ch5x5, kernel_size=5, padding=2)
        )

        # 3x3 maxpool -> 1x1 conv branch
        self.branch4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),
            BaseConvBlock(in_channels, pool_proj, kernel_size=1)
        )
...
class GoogLeNet(nn.Module):
    def __init__(self, labels=21):
        ...
        self.conv_2_1 = BaseConvBlock(
            in_channels=64,
            out_channels=64,
            kernel_size=1
        )
        
        self.conv_2_2 = BaseConvBlock(
            in_channels=64,
            out_channels=192,
            kernel_size=3,
            padding=1
        ...
        self.linear = nn.Linear(in_features=1024, out_features=labels)
